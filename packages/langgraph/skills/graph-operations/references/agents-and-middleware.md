...
List the classes in `middleware: [...]` and register their options in **`forFeature`'s `{ providers }`** (see the DI gotcha). Middleware named on an agent resolves **within the owning graph's feature module** — two graphs may name the same middleware class with different scope-level options and each gets its own instance bound to its own scope; no subclass-per-graph disambiguation needed. Corollary: to override an agent-listed middleware class (`{ provide: BudgetMiddleware, useClass: ... }`), the override must live in that graph's `forFeature` providers — an override registered elsewhere in the app does not reach the graph.
```ts
