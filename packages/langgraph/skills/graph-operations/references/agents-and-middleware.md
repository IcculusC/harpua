...
List the classes in `middleware: [...]` and register their options in **`forFeature`'s `{ providers }`** (see the DI gotcha). Middleware resolves **within the owning graph's feature module first** (flat container fallback), so two graphs may name the same middleware class with different scope-level options — each graph gets its own instance bound to its own scope. No subclass-per-graph disambiguation needed.
```ts
