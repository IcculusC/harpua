...
> **First, one intent check that overrides the table:** are you building a **model↔tools agent loop** — the model calls tools and you loop until it's done (a chat / ReAct / tool-using agent; anything that needs to "stop after N turns", avoid `GraphRecursionError`, retry the model, or trim/compact history)? Then read **`packages/langgraph/skills/graph-operations/references/agents-and-middleware.md`** and use the `@LangGraphAgent` preset — do NOT hand-roll it as a `@LangGraph` graph plus a model-calling node. This is the single most common thing agents wrongly build from scratch.
